...
- New 3D saves now retain all 240 generated NPC employment assignments through strict persistence
  validation: each worker is deterministically placed at a structure that exposes the station role
  their job requires, rather than causing the valid 3D save section to be rebuilt.
...
- Funny-level language samples now render their requested voice without changing the persisted
  language or notifying listeners. Repeated active-tab and language-refresh notifications no longer
  restart an already visible Farm surface or re-run panel synchronization, allowing the initial
  render callback to run.
