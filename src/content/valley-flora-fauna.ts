  ...
  CROP_FAMILIES.forEach((family, familyIndex) => {
    family.cultivars.forEach((cultivar, cultivarRank) => {
      ...
      rows.push({
        ...
        tags: [family.guild, `family:${family.cropFamily}`, role, `cultivar:${cultivarSlug}`, `harvest:${cropHarvestMethod(family.cropFamily)}`],
  ...
  for (const family of ORCHARD_FAMILIES) {
    for (const cultivar of family.cultivars) {
      ...
      rows.push({
        ...
        tags: [family.climate, `form:${family.plantForm}`, `plant:${plantSlug}`, `pollination:${pollination}`, `harvest:${slugify(harvest)}`],
